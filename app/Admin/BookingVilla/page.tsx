...
const getVilla = async () => {
  const res = await prisma.bookingVilla.findMany({
    select: {
      id: true,
      userId: true,
      villaId: true,
...
const BookingVilla = async () => {
  const villas = await getVilla();

  return (
    <div className="bg-white shadow-md rounded-md p-4">
      <h1 className="text-2xl font-bold mb-4">Management  Booking Villa</h1>
      ...
      <table className="w-full overflow-x-auto">
        <thead className="bg-gray-50 text-gray-700 uppercase">
          <tr>
            ...
            <th className="px-4 py-3 text-left">Tangal Check In</th> 
